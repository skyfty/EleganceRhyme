...
class PDFS extends StatefulWidget {
  final String? path; //书籍pdf地址
  final String? dynasty; //朝代
  final String? author; //作者

  final String? sound_file; //系统录音
  final String? token;
  final String? book_name;
  final String? nickname; //当前登录用户昵称
  final String? username; //当前登录用户的username
...
//播放量
  ...
  //   final recordingUrl = await _uploadAI(file);
  ...
  // }
  //
  // Future<String> _uploadAI(File file) async {
  //   final url = Uri.http('192.168.33.212:9977', '/api');
  //   final request = http.MultipartRequest('POST', url)
  //     ..files.add(await http.MultipartFile.fromPath('file', file.path))
  //     ..fields['targetText'] = '作者';
  //
  ...
  //       print(data);
  //       print(11111);
  //       if (data['is_matched'] == false) {
  //         print('Uploading failed: ${data['msg']}');
  //         throw Exception('Uploading failed: ${data['msg']}');
  //       }
  //       final uploadedFilePath = await _uploadRecording(file);
  //       return uploadedFilePath;
  //     } else {
  //       throw Exception('Failed to upload recording: ${response.statusCode} - ${response.reasonPhrase}');
  //     }
  //   } catch (e) {
  ...
  //   final url = Uri.http('${apiVar.serverIp}', '/FaElegance/public/testyuyin/1.php');
  //   final request = http.MultipartRequest('POST', url)
  //     ..files.add(await http.MultipartFile.fromPath('file', file.path));
  //
  //   try {
  ...
  Future<void> _stopRecording() async {
    ...
    final recordingUrl = await _uploadAI(File(_currentFilePath));
    final newRecording = VoiceRecording(
  ...
  // 在页面的 State 类中添加 _isLoading 状态变量
  bool _isLoading22 = false;
  String removeHTMLTags(String text) {
    return text.replaceAll(
        RegExp(r'<[^>]+>|&nbsp;'), ''); //要去除字符串中的HTML标签和 &nbsp;
    // return text.replaceAll(RegExp(r'<[^>]+>'), '');
  }

  Future<String> _uploadAI(File file) async {
    setState(() {
      _isLoading22 = true; // 设置 _isLoading 为 true，显示加载
    });

    final url = Uri.http('www.zlgccn.com:9956', '/api');
    final targetText = removeHTMLTags('${widget.path}');
    print(targetText);
    final request = http.MultipartRequest('POST', url)
      ..files.add(await http.MultipartFile.fromPath('file', file.path))
      ..fields['targetText'] = targetText;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['is_matched'] == false) {
          print('Uploading failed: ${data['msg']}');
          Fluttertoast.showToast(msg: '未识别出正确语音，请重试');
          throw Exception('Uploading failed: ${data['msg']}');
        } else if (data['is_matched'] == true) {
          Fluttertoast.showToast(msg: '${data['msg']}');
          final uploadedFilePath = await _uploadRecording(file);
          setState(() {
            _isLoading22 = false; // 设置 _isLoading 为 false，隐藏加载
          });
          return uploadedFilePath;
        } else {
          Fluttertoast.showToast(msg: '未识别出正确语音，请重试');
        ...
        throw Exception(
            'Failed to upload recording: ${response.statusCode} - ${response.reasonPhrase}');
      }
    } catch (e) {
      print(121);
      setState(() {
...
print(responseData);
  ...
  //测试
  // String _stripHtmlTags(String htmlString) {//阅读正文转换
  //   RegExp htmlRegExp =
        ...
        children: <Widget>[
          ...
          Positioned(
            ...
            child: Container(
              height: 160,
              decoration: BoxDecoration(
                color: ThemeManager.themes[colorsID.colors].PdfAppBarColor,
                            ...
                            if (audioList.isNotEmpty &&
                              ...
                              Text(
                                ...
                                style: TextStyle(
                                  fontSize: 20,
                                  color: ThemeManager
                                      .themes[colorsID.colors].PdfTextColor,
                                ),
                              ),
                            if (audioList.isNotEmpty &&
