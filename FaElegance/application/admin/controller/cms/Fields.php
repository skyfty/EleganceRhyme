...
class Fields extends Backend
{
    ...
    public function _initialize()
    {
        ...
        $typeList = array_diff_key(Config::getTypeList(), array_flip(['custom']));
        ...
        $this->assignconfig('withoutModelList', ['channel', 'page', 'special', 'block', 'archives']);
        $this->assignconfig('contributeFields', \app\admin\model\cms\Fields::getContributeFields($source));
        $this->assignconfig('publishFields', \app\admin\model\cms\Fields::getPublishFields($source));
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            } elseif (in_array($source, ['channel', 'page', 'special', 'block', 'archives'])) {
    ...
    public function edit($ids = null)
    {
        if (is_numeric($ids)) {
            ...
            $setting['filterlist'] = $setting['filterlist'] ?? [];
            $setting['titlelist'] = $setting['titlelist'] ?? [];
            $title = $setting['titlelist'][$name] ?? $title;
    ...
    protected function renderTable()
    {
        ...
        $source_id = $this->request->param('source_id', 0);
        $fieldsList = \app\admin\model\cms\Fields::where('source_id', $source_id)->where(function ($query) {
        ...
        foreach ($fieldsList as $index => $item) {
            $item->visible(['name', 'title']);
        }

        //主表字段
        $masterFieldsList = [];
        $notAllowFields = ['id', 'model_id', 'user_id', 'style', 'views', 'comments', 'likes', 'dislikes', 'weigh', 'isguest', 'iscomment', 'createtime', 'updatetime', 'publishtime', 'deletetime'];
        $prefix = \think\Config::get('database.prefix');
        $fieldList = Service::getTableFields("{$prefix}cms_archives");
        foreach ($fieldList as $index => $item) {
            if (!in_array($item['name'], $notAllowFields)) {
                $masterFieldsList[] = $item;
            }
        }

        $this->assignconfig('diyFieldsList', array_merge($masterFieldsList, $fieldsList));
    }
    ...
    public function multi($ids = "")
    {
        ...
        if (isset($paramsArr['iscontribute']) && !is_numeric($ids)) {
            if (!$ids || !in_array($ids, \app\admin\model\cms\Fields::getContributeFields($source))) {
                $this->error('参数错误');
            }
            $source_id = $this->request->param('source_id', 0);
            $model = \app\admin\model\cms\Modelx::get($source_id);
        ...
        if (isset($paramsArr['ispublish']) && !is_numeric($ids)) {
            if (!$ids || !in_array($ids, \app\admin\model\cms\Fields::getPublishFields($source))) {
                $this->error('参数错误');
            }
            $source_id = $this->request->param('source_id', 0);
            $model = \app\admin\model\cms\Modelx::get($source_id);
        ...
        if (isset($paramsArr['isfilter']) && !is_numeric($ids)) {
            ...
            $setting = $model['setting'];
            $filterfields = $setting['filterfields'] ?? [];
